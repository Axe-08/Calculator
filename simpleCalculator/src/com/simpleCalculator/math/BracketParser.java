...
public class BracketParser {
	...
	public static Boolean isBracketAvailable(StringBuilder exp) throws Exception {
	...
	public static int findLastOpenBracket(StringBuilder exp,int closingBracketIndex) {
	...
	public static String bracketSolving(StringBuilder exp) {
		try {
			while(isBracketAvailable(exp)) {
				...
				int lastOpenBracket = findLastOpenBracket(exp,firstClosingBracket);				
				String brackExp = exp.substring(lastOpenBracket+1,firstClosingBracket);	//sending expression inside bracket to evaluator
				float brackResult = Evaluator.evaluate(brackExp);
				exp.replace(lastOpenBracket,firstClosingBracket+1,Float.toString(brackResult)); //replaces the bracket and inner expression with result for further calculation.
			}
		} catch (Exception e) {
